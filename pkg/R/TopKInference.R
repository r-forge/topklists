...
{
return(list(j0_est=length(Idata), reason.break="Idata is identity", Js=NA, v.vector=v))
...
if(v.last<=1 | (Js[k]-v.last+1)<=0){j0_est=NA} else {if ((floor(k/2)<(k/2)) & k>2) {j0_est = ceiling(Js[k-2]+0.5*v[k-2]-1)}else if ((floor(k/2)==(k/2)) & k>1){j0_est = ceiling(Js[k-1]+0.5*v[k-1]-1)} else{j0_est=NA}}
