...
\section{Specification of $\delta$ and calculation of \texttt{maxK} for multiple ranked lists}
...
<<fig=F,include=TRUE,label=deltaplot>>=
a=deltaplot(lists, maxd=nrow(lists), subset.plotted=NA)
@
...
<<>>=
a=deltaplot(lists, maxd=nrow(lists), subset.plotted=NULL)
@
