...
The example dataset comprises three lists of microarray results (differential gene expression) from three breast cancer studies (for accession numbers see Table \ref{tab:data}):
\begin{itemize}
    \item MicroArray Quality Control Phase II Project (2010)\footnote{http://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE20194} - data called \textit{MDACC} \cite{shi}
    \item Strong Time Dependence of the 76-Gene Prognostic Signature (2007)\footnote{http://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=gse7390} - data called \textit{TransBig} \cite{desmedt}
    \item A Clinically Relevant Gene Signature in Triple-Negative and Basal-Like Breast Cancer (2011)\footnote{http://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE31519} - data called \textit{Pusztai} \cite{rody}
\end{itemize}
...
\begin{tabular}{|c|c|c|}
  ...
  NAME & PubMed ID & GEO \\ \hline
  MDACC & 20676074 & GSE20194 \\
  TransBig & 17545524 & GSE7390 \\
  Pusztai & 21978456 & GSE21978456 \\
  \hline
\end{tabular}
\caption{Example dataset: names, PubMed IDs and GEO accession numbers}
\end{center}
\end{table}
...
res$maxK
@
The example dataset contains no missing values, i.e. all three lists share the same genes. Therefore the underlying space is identical to  the original dataset and \texttt{space} can be set to NULL.
<<>>=
topK = as.list(breast[1:res$maxK, ])
pd = CEMC(topK, res$maxK, space=NULL)
pd$topK
@
Resulting top-k genes are ordered according to their probability.
