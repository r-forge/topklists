...
@
``space'' is explicitly specified; platform-dependent
...
plotBorda(outBorda, K=40, sub="(a)") 
@
plots scores from platform-dependent analysis

<<label=bordaPlot2, include=FALSE>>=
plotBorda(outBorda2, sub="(b)") 
@
plots scores from top-k space analysis
...
<<>>=
set.seed(1234)
outCEMC=CEMC(input, space)  
@
''space" is explicitly specified; platform-dependent

<<>>=
set.seed(1234)
outCEMC1 = CEMC(input, k=30) 
@
''space" is not specified, so it is the same as common space (1-40)

<<>>=
set.seed(1234)
outCEMC2 = CEMC(input, space=input) 
@
"space=input" indicates that this is the top-k space.
<<>>=
