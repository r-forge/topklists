...
potential aggregate top-40 lists is more than $8 \times 10^{47}$, enumerate all of them to find the 
...
objects 16-20, the results were more of a mixture between these two groups, but with the correct
relative orders preserved within each group. The results are quite similar among the four scoring 
functions, with ``l2norm'' being a little bit better in this example. This result is not surprising
due to degradation of ranking information beyond the top 10 (both groups of objects are ranked 
10-15 and 16-20 in two of the lists). Such information degradation can be seen by plotting the 
Borda scores.   
\\[2ex]
...
at rankings 10--11, and that the changes of the scores also slow down for the lower ranked objects, 
signaling degradation of information. This can be similarly observed for the scores under the 
top-$k$ space assumption (Figure~\ref{borda}(b)). \\
%% NEEDS SERIOUS REWRITE!!!

%<<label=bordaPlot1, include=FALSE>>=
plot(outBorda, K=40, sub="(a)") plot of scores from platform-dependent analysis
%@
%<<label=bordaPlot2, include=FALSE>>=
plot(outBorda2, sub="(b)") plot of scores from top-k space analysis
%@
...
Compared to the results from Borda, MC appears to be superior for this
data set. Nevertheless, the stationary probabilities also reflect information degradation,
which is visualized using the plotting function as described in the following.
\\[2ex]
...
\begin{figure}[h]
\begin{center}
\includegraphics[scale=0.6]{equil.pdf}\\
\end{center}
...
parameters. In fact, the output of CEMC contains the set of tuning parameters used in the current run. If the user
...
each tuning parameter separately, if the user wishes.
