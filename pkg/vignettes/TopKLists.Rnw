...
We apply the Borda, MC, and CEMC algorithms to the breast cancer data to illustrate their usage in practical application.
<<>>=
data(breast)
@
...
\caption{\texttt{TopKListsGUI} Venn diagram and Venn table results of the TopKListsGUI
\label{venn}}
\end{figure}
