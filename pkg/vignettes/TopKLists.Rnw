...
Here the three MC algorithms and the CEMC method are applied to the breast cancer dataset:
<<>>=
data(breast)
@
...
Note: The results indicate that CEMC performs better with  a smaller distance
between the aggregate and individual lists (\ref{fig:aggplot}).
