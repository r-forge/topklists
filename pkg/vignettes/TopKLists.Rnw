...
\item \texttt{TopKSpace} provides several rank aggregation techniques (Borda, Markov chain, and Cross-entropy Monte Carlo) which allow the combination of input lists of different lengths (space) as well as missing rank information;
\item \texttt{TopKGraphics} provides a collection of graphical tools for visualization of the inputs to and the outputs from the other modules.
\end{enumerate}
...
<<label=truncPlot, include=FALSE, tidy=TRUE>>=
...
\begin{figure}[h]
\begin{center}
<<label=trunc,echo=FALSE, out.width='8cm', out.height='8cm', tidy=TRUE>>=
<<truncPlot>>
@
\end{center}
...
\begin{tabular}{|c|c|c|c|}
  ...
  MDACC & 20676074 & \href{http://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE20194}{GSE20194} & \cite{shi}\\
...
<<fig.keep='none', out.width='4cm', out.height='5cm'>>=
a=deltaplot(breast) 
@

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{deltaplot_completedataset.png}\\
...
\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{deltaplot_subsetlists200.png}\\
...
platform-dependent approach, there is a big gap in the Borda score in the rankings
from 10-11,
...
\begin{figure}[h]
\begin{center}
<<label=borda,echo=FALSE, out.width='8cm', out.height='8cm'>>=
<<bordaPlot>>
@
\end{center}
...
The difference in the Borda scores can be clearly seen in the resulting graph (Figure~\ref{borda}).
\\[2ex]
...
\begin{figure}[h]
\begin{center}
<<label=equil2,echo=FALSE, out.width='8cm', out.height='8cm'>>=
<<equil>>
@
\end{center}
...
res$maxK
@
Each of the three lists is truncated to the 14 mostly highly ranked genes. The set of genes contained in each list is not identical and each list can contain a different set of 14 top genes. Therefore the underlying space will be a set of all the genes present in these 3 trucated lists. In our case there are 22 unique genes.
<<>>=
