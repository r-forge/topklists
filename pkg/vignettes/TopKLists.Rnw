...
\tableofcontents
\newpage
\section{Introduction and example dataset}
...
<<truncPlot, out.width='8cm', out.height='8cm', fig.pos="ht!", fig.cap="Estimation of $j_0$ for different values of $v$", fig.align='center'>>=
...
These three algorithms are implemented in three top-level functions within TopKSpace: Borda, MC, 
and CEMC. There are also two plotting functions: plotBorda and plotMC that help the user to visualize 
...
Although this example is contrive, it is realistic in terms of the lengths and the number of top-K lists.
...
objects 41-50 were not selected in the top-K of the two corresponding lists. Fifteen of these 40 objects 
...
21-30. In the following subsection, we will demonstrate the use of the three classes of algorithms  (Borda, MC, CEMC) on aggregate the ranking information
...
\subsubsection{Borda algorithm}
Four Borda scores are implemented in the TopKSpace Borda function, namely the arithmetic mean (ARM), median (MED),
geometric mean (GEO), and L2-norm (L2N). The output is a list with two elements: ``Top-K'' provides
the aggregate rankings and ``Scores'' gives the corresponding Borda scores
for each of the four functions.
Of the three arguments in Borda, only the first one ({\em input}) is required. If argument {\em space} is not
provided, then the underlying space is assumed to be common among all top-k lists implicitly, and the union of
the top-k lists is treated as the common space. If argument {\em K} is not specified, then the full ranked
 list 
(all elements in the union) is outputted.
 

<<>>=
outBorda=Borda(input,space,K=50) # "space" is explicitly specified; platform-dependent
@

<<>>=
outBorda1=Borda(input) #"space" is not specified; all lists are assumed to come from the common space (objects 1-40)

@

<<>>=
outBorda2=Borda(input,space=input) # "space = input" indicates that this is the top-K space
@
...
Comment 1: Objects 1-40, union of the three top-k lists, are contained in all three spaces, whereas
 objects 41-50 are not in any of the list. As such, the results for which the spaces were explicitly
specified should be the same as those with the common space assumption, which was exactly what we saw above. On the 
other hand, the platform-dependent results were different from the results
assuming top-k spaces (i.e., all lists were full ranked lists).
...
platform-dependence approach, a large gap appears between the Borda scores 
...
top-$k$ space assumption (Figure~\ref{fig:borda}(b)). \\


<<borda, fig.cap='Plotting Borda\'s scores', fig.subcap=c('platform-dependent analysis','top-k space analysis'), out.width='.49\\linewidth', fig.pos='ht!', fig.align='center'>>=
plotBorda(outBorda, K=40) #plot scores from platform-dependent analysis
plotBorda(outBorda2) # plot scores from top-k space analysis
@




Note that the first argument of the plotBorda function is the output from the Borda function, which is 
...
Three Markov Chain (MC) algorithms are implemented in the \textit{MC} function in
\texttt{TopKSpace}: MC1 (spam sensitive), 
MC2 (majority rule), and MC3 (proportional). 
Among them, MC3 may be more appropriate for multi-platform omics problems given the potential of unique features of each data type.
The output is a list with two elements for each of the MC algorithms(aggregate top-k list: MC1.TopK, MC2.TopK, or MC3.TopK; stationary probability distribution: MC1.Prob, MC2.Prob, or MC3.Prob). 
Of the arguments in MC, only the first one, {\em input}, is required. If argument {\em space} is not provided, it is assumed that the underlying space is common among all top-k lists, and the union of
the top-k list is supplied as the common space. If argument {\em K} is not specified, then the full ranked list (all elements in the union) is outputted.
The other two arguments ($a$ and $delta$) are tuning parameters, whose default values are shown to work well in a number of examples.


<<>>=
outMC=MC(input,space) # "space" is explicitly specified; platform-dependent
@


<<>>=
outMCa=MC(input,K=30) # "space" is not specified, so it is the same as common space (1-40)
@


<<>>=
outMCb=MC(input,space=input) # "space = input" indicates that this is the top-K space
@
...
Comment 1: 
Similar to the discussion for running the Borda function, 
...
top-k space (there were large discrepancies in the aggregated top-k lists).
...
Comment 2: All three MC algorithms ranked objects 1-10, in that order, in the top-10. MC2 also ranked the next two groups, 11-15 and 16-20 in the correct 
order. However, the remaining objects (21-40) were ranked in the reverse of
the expected order. MC3, on the other hand, had all the correct ordering 
...
\\[2ex]
\textbf{Plotting the stationary probabilities}\\
...
for MC2, there was considerable information for ranking objects in the top 20,
...
On the other hand, for MC1, there was less information for ranking
objects beyond the top 10 compared to MC2, thus objects 11-20 were not ranked
correctly. MC3 appears to be a compromise between MC1 and MC2, leading to a better 
aggregate ranked 
list.
...
Note that the first argument of the plotMC function is the output from the MC function, which is
mandatory. If the number of ranked objects (second argument) is not specified, the plot will show
probabilities for all objects in the union set.

\subsubsection{Running the CEMC algorithm}
The Order Explicit Algorithm (OEA) in \cite{linding} is implemented as the CEMC function in TopKSpace.
Of the three main input arguments in CEMC, only the first one ({\em input}) is required. If argument {\em space}
is not provided, then all the underlying spaces are assumed to be a common one, and the union of 
the input top-k lists is supplied as the common space. If argument {\em k} is not specified, then the full 
...
The default specified in the CEMC function constitutes a set of 
values that seem to work well for the examples/data that we have analyzed, but it is always a good idea
...
parameters. In fact, the output of CEMC contains the set of tuning parameters used in the current run. If  user
is unsure of what to use, they are recommended to run CEMC without setting any parameter and use 
the parameter file in the output ({\em input.par}) to fine tune the parameters and use them as input
for a more refined analysis. For the convenience of user, after the parameter values are 
modified, the file can be directly supplied as an input argument without having to enter the value of 
each tuning parameter separately, if user wishes.
Other than {\em input.par}, there are two other elements in the output list. 
One is the 
aggregate top-k list ({\em TopK}). The other is the multinomial probability matrix ({\em ProbMatrix};
each column 
represents the probability vector of a multinomial distribution and thus sum to 1, albeit with
small rounding
errors), from which the aggregate top-k list is determined.

<<>>=
set.seed(1234)
outCEMC=CEMC(input, space)  # "space" is explicitly specified; platform-dependent
@


<<>>=
set.seed(1234)
outCEMC1 = CEMC(input, k=30) # "space" is not specified, so it is the same as common space (1-40)
@


<<>>=
set.seed(1234)
outCEMC2 = CEMC(input, space=input) # "space=input" indicates that this is the top-k space.
@

<<>>=
sum(outCEMC$ProbMatrix[,1:5]- outCEMC1$ProbMatrix[,1:5])
sum(abs(outCEMC$TopK[1:30]- outCEMC2$TopK[1:30]))
@

Comment 1: If we set the same random seed for running the first two examples, then their results should
be identical (apart from potential rounding errors). On the other hand, the common-space results are different from those assuming top-k
space. 

<<>>=
list(outCEMC$TopK[1:30], outCEMC1$TopK[1:30], outCEMC2$TopK[1:30])
outCEMC$ProbMatrix[1:5,1:5] 
outCEMC1$ProbMatrix[1:5,1:5]
@

Comment 2: From the submatrix shown above for outCEMC, it can be seen that item one is ranked no. 1, since the probability vector in the first column for item one is almost 1, but very small for the rest of the items. Similarly, item 2 is ranked no. 2, and so on.
On the other hand, for outCEMC1 with top-k space assumption, the 5th item is no longer ranked no.5, different from the platform-dependence results.
This discrepancy can also be seen from the above Top-K outputs.

<<>>=
outCEMC$input.par
@

Comment 3: This file contains all the tuning parameter values (defaults) for running CEMC. User may
...
based on the modified Kendall distance (MKD) \cite{lin} in the TopKSpace module. The first 
function is Kendall, which computes the weighted sum of 
MKD between an aggregate top-k list with the input lists. 
...
{\em w}.
The output of Kendall is the MKD, a single number. However, it would be much more meaningful
to compare MKDs accross results from different algorithms to assess their relative
performances. This can be achieved by invoking the plotKendall function, which not only 
provide a vector of MKDs but also provide a plot of the MKDs for ease of comparison visually.
There are also two mandatory input arguments, {\em input} and {\em all.aggregates},
where all.aggregates is a list comprising of aggregate top-k lists from different 
algorithms to be compared. 

The following command will compute the MKD for the aggregate list form the  
Borda algorithm "ARM".

<<>>=
kendallModified(input,outBorda$TopK[,1],space)
all.aggregates=list(outBorda$TopK[,1],outBorda$TopK[,2],outBorda$TopK[,3],outBorda$TopK[,4],outMC$MC1.TopK,outMC$MC2.TopK,outMC$MC3.TopK)
@
...
plotKendall(input,all.aggregates,space,algorithm=c("ARM","MED","GEO","L2N","MC1","MC2","MC3"))
@
Runing the Kendall function returns the MKD for the aggregate list from the
Borda algorithm "ARM", whereas the plotKendall function returns a vector of MKDs, with
the first one matching the output from the Kendall function.
Figure~\ref{fig:plotKendall} is the graphical component of the plotKendall output.
As we can see, the range on the y-axis is very small, indicating similar performance of 
all algorithms, especially those from Borda. The result from MC3 seems to perform a bit 
...
Here the Cross Entropy Monte Carlo method on the breast cancer dataset is applied (introduced in Section~\ref{sec:introduction}):
<<>>=
data(breast)
@
...
To open the GUI using the breast cancer example data introduced in \ref{sec:introduction}, run
<<eval=FALSE>>=
TopKListsGUI(breast)
@ 
...
As shown in Figure~\ref{aggmap}, the best overlap was found when comparing the MDCC and Transbig lists against the Pusztai list, for $\delta = 6$ (see Section~\ref{sec:introduction} for a description of the example datasets). The estimated $\hat{k}$ is 14, with 12 genes in the top-$k$ lists of at least two datasets. In the visualization of Transbig with respect to MDCC, 10 genes are grey-shaded. Most consistent are the ESR1 and TBC1D9 genes, occupying the first two positions in all three lists (with a distance of 0 in both MDCC and TransBig compared to Pusztai).\\
...
\bibitem{desmedt} Desmedt C., Piette F., Loi S., Wang Y. et al. (2007). Strong time dependence of the 76-gene prognostic signature for node-negative breast cancer patients in the TRANSBIG multicenter independent validation series. \emph{Clin Cancer Res} 1;13(11):3207-14. PMID: 17545524.
...
\bibitem{shi} Shi L., Campbell G., Jones W.D., Campagne F. et al. (2010). The MicroArray Quality Control (MAQC)-II study of common practices for the development and validation of microarray-based predictive models. \emph{Nat Biotechnol};28(8):827-38. PMID: 20676074.

\bibitem{tabchy} Tabchy A., Valero V., Vidaurre T., Lluch A. et al. (2010). Evaluation of a 30-gene paclitaxel, fluorouracil, doxorubicin, and cyclophosphamide chemotherapy response predictor in a multicenter randomized trial in breast cancer. \emph{Clin Cancer Res} 1;16(21):5351-61. PMID: 20829329
