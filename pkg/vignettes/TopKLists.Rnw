...
There are three common assumptions about the underlying spaces: {\em common-space} (all top-\emph{k} lists come from a single common space), {\em assessor-dependent} (using the known spaces from which the top-\emph{k} lists were generated), and {\em top-k-space} 
(treating each top-\emph{k} list as its own space). Since assessor-dependent represents the true underlying scenario, this method is recommended if such information is available.
...
21-30. In the following subsection, we will demonstrate the use of the three classes of algorithms (Borda, MC, CEMC) for aggregation of these three ranked lists.
...
Four Borda scores are implemented in the Borda function of TopKSpace, namely the arithmetic mean (ARM), median (MED),
...
<<>>=
outBorda=Borda(input,space,K=50) # "space" is explicitly specified; assessor-dependent
@
...
<<>>=
outBorda2=Borda(input,space=input) # "space = input" indicates that this is the top-\emph{k} space
@
...
Comment 1: Objects 1-40, union of the three top-\emph{k} lists, are contained in all three spaces, whereas
...
other hand, the assessor-dependent results were different from the results
...
assessor-dependence approach, a large gap appears between the Borda scores 
...
<<borda, fig.cap='Plotting Borda\'s scores', fig.subcap=c('assessor-dependent analysis','top-k space analysis'), out.width='.49\\linewidth', fig.pos='ht!', fig.align='center'>>=
plotBorda(outBorda, K=40) #plot scores from assessor-dependent analysis
plotBorda(outBorda2) # plot scores from top-\emph{k} space analysis
@
...
Three Markov Chain (MC) algorithms are implemented in the MC function in
...
The output is a list with two elements for each of the MC algorithms(aggregate top-\emph{k} list: MC1.TopK, MC2.TopK, or MC3.TopK; stationary probability distribution: MC1.Prob, MC2.Prob, or MC3.Prob). 
Of the arguments in MC, only the first one, {\em input}, is required. If argument {\em space} is not provided, it is assumed that the underlying space is common among all top-\emph{k} lists, and the union of
...
The other two arguments ($a$ and $delta$) are tuning parameters, whose default values were shown to work well in a number of examples.


<<>>=
outMC=MC(input,space) # "space" is explicitly specified; assessor-dependent
@
...
<<>>=
outMCb=MC(input,space=input) # "space = input" indicates that this is the top-\emph{k} space
@
...
parameters. In fact, the output of CEMC contains the set of tuning parameters used in the current run. If  users
are unsure of what to use, they are recommended to run CEMC without setting any parameter and use 
...
<<>>=
set.seed(1234)
outCEMC=CEMC(input, space)  # "space" is explicitly specified; assessor-dependent
@
...
<<>>=
set.seed(1234)
outCEMC2 = CEMC(input, space=input) # "space=input" indicates that this is the top-\emph{k} space.
@
...
be identical (apart from potential rounding errors). On the other hand, the common-space results are different from those assuming top-\emph{k}
space. 
...
On the other hand, for outCEMC1 with top-\emph{k} space assumption, the 5th item is no longer ranked no.5, different from the assessor-dependence results.
This discrepancy can also be seen from the above TopK outputs.
...
where all.aggregates is a list comprising of aggregate top-\emph{k} lists from different 
...
data(breast)
@
Now the overall top-\emph{k} list length \texttt{maxK} is calculated using the \texttt{j0.multi} function
<<>>=
res = j0.multi(breast, d=6, v=10)
res$maxK
@
Each of the three lists is truncated to the 14 most highly ranked genes. The set of genes contained in each list is not identical and each list can contain a different set of 14 top genes. 
Since each of the top-14 gene lists come from the common set of 917 genes, one way to set the common underlying space, without loss of information, is to make it the set of all the genes present in these 3 truncated lists. 
In this case there are 22 unique genes. 
<<>>=
...
The Venn diagram, and the respective Venn table, show overlaps of objects in the top-\emph{k} lists for all analyzed input lists. Asterisk-highlighted objects are those from the final aggregated list, which are the result of \texttt{TopKSpace}'s output.
...
\item{\texttt{Venn-diagram and Venn-table}}:\\ The outputted Venn diagram and Venn table show overlaps of objects in the top-\emph{k} lists of all input lists in the analysis. Asterisk-highlighted genes are genes that form the final aggregate list as returned by the \texttt{TopKSpace} module.
