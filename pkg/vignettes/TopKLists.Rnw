...
These three algorithms are implemented in three top-level functions within \texttt{TopKSpace}: \texttt{Borda.scores}, \texttt{MC}, 
...
Comment 2: From the submatrix shown above for \texttt{CEMC}, it can be seen that item one is ranked no. 1, since the probability vector in the first column for item one is almost 1, but very small for the rest of the items. Similarly, item 2 is ranked no. 2, and so on.
