...
and \texttt{CEMC}. There are also two plotting functions: \texttt{plotBorda} and \texttt{plotMC} that help the user to visualize 
...
Note that the first argument of the \texttt{plotBorda} function is the output from the Borda function, which is 
...
The output is a list with two elements for each of the \texttt{MC} algorithms(aggregate top-\emph{k} list: MC1.TopK, MC2.TopK, or MC3.TopK; stationary probability distribution: MC1.Prob, MC2.Prob, or MC3.Prob). 
...
Note that the first argument of the \texttt{plotMC} function is the output from the \texttt{MC} function, which is
mandatory. If the number of ranked objects (second argument) is not specified, the plot will show
...
Comment 2: From the submatrix shown above for \texttt{outCEMC}, it can be seen that item one is ranked no. 1, since the probability vector in the first column for item one is almost 1, but very small for the rest of the items. Similarly, item 2 is ranked no. 2, and so on.
On the other hand, for \texttt{outCEMC1} with top-\emph{k} space assumption, the 5th item is no longer ranked no.5, different from the assessor-dependence results.
This discrepancy can also be seen from the above \texttt{TopK} outputs.

<<>>=
outCEMC$input.par
@

Comment 3: This file contains all the tuning parameter values (defaults) for running \texttt{CEMC}. User may
...
performances. This can be achieved by invoking the \texttt{plotKendall} function, which not only 
...
Borda algorithm "ARM", whereas the \texttt{plotKendall} function returns a vector of \texttt{MKD}s, with
the first one matching the output from the \texttt{Kendall} function.
Figure~\ref{fig:plotKendall} is the graphical component of the \texttt{plotKendall} output.
