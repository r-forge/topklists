...
There are two basic statistical tasks: (i) Identification of the top-\emph{k} most conforming items. For this task, two lists are analyzed together (multiple lists in a pairwise manner). (ii) Calculation of a consolidated top-\emph{k} sublist with a new optimized ordering of the conforming items from two or more lists. For long ranked lists, (i) is a prerequisite of (ii) because for any kind of rank aggregation the top-$k$ list lengths of the individual input lists need to be specified.
...
(2) \texttt{TopKSpace} provides several rank aggregation techniques (Borda, Markov chain, and Cross-entropy Monte Carlo) which allow the combination of input lists of different lengths (space) as well as missing rank information;
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth]{TopKInferenceFlowmap.pdf}\\
...
The following plot summarizes the obtained estimation results $\hat{j}_0$ for the specified range of pilot sample sizes $\nu$ and the assumed point of degeneration $j_0=31$. 
...
@
Specification of the $\delta$-values for all possible pairs of the three input lists, $L1$, $L2$, and $L3$ is possible by inspecting \texttt{deltaplot}. 

<<fig=F>>=
a=deltaplot(lists, deltas= c(1:nrow(lists)))
@
The parameter \texttt{subplot} magnifies a selected range of small $\delta$-values in a subplot, defined by parameter \texttt{perc.subplot}.

<<fig=F>>=
a=deltaplot(lists, deltas=c(1:nrow(lists)), subplot=TRUE, perc.subplot=50)
@
To see the results for a subset of the original list data, specify the length using the parameter \texttt{subset.lists}:
<<fig=F>>=
a=deltaplot(lists, deltas=c(0:2), subset.lists=5)
@
In the obtained \texttt{deltaplot} it can be observed that the switching of the reference list from, e.g. $L1$ to $L2$ or vice versa, has little impact on the graph. What can be clearly seen are differences in the point configurations (functions) when one of the lists ($L3$) is a random sample, all of length $N$=14. 
...
res
@
In our example, \texttt{maxK=12}  and, as expected, no overlap was found with list L3.
...
The \textbf{deltaplot} is an exploratory graph, designed to help the user with selection of the distance parameter $\delta$ in the \texttt{TopKListsGUI} interface. 
...
From the \texttt{deltaplot} for the first two lists of the breast cancer dataset (Figure \ref{deltaplot_complete}), we cannot clearly see what $\delta$ should be used, because there is no visible point where \texttt{deltaplot}'s decrease changes.
...
To decide about the suitable choice of $\delta$, we have to limit the \texttt{deltaplot} calculation on a smaller subset of the investigated lists, let's say first 200 objects.

<<fig=F>>=
a=deltaplot(breast, subset.lists=200)
@

\begin{figure}[H]
\centering
\includegraphics{deltaplot_subsetlists200_3.png}\\
\caption{Deltaplot for lists TransBig vs. MDCC for first 200 objects} 
\label{deltaplot_subset200}
\end{figure}

From the Figure \ref{deltaplot_subset200} we can already see, that after let's say $\delta=5$ the decrease of \texttt{deltaplot} is not as dramatical as for lower values of $\delta$. Therefore we should concentrate on results with $\delta$ set between 4 and 10. 

The \textbf{aggregation map} can be characterized as follows: We define an index $p=1,2,\ldots$ and combine $\ell-1$ aggregation levels (groupings of truncated lists) in one display: 
...
(a) The \textbf{membership} of an individual item in the top-$k$ lists, \textit{yes} is denoted by the color `grey' and \textit{no} by the color `white'. 
(b) The \textbf{distance} $d$ of the rank of an individual item $o\in L^0$ from its position in the other list, is denoted by a triangle color scaled from `red' \textit{identical} to 
...
ranked higher in $L$ with respect to $L^0$. (c) The rectangular of a symbol takes on the color `grey' when the \textbf{percentage} of $d \leq \delta$ across the columns of a group is above 
some prespecified threshold, and `white' otherwise.


The \textbf{Venn diagram}
...
\subsection{Introduction of the example dataset}

The example dataset comprises three lists of microarray results (differential gene expression) from breast cancer studies. 
<<>>=
data(breast)
str(breast)
@
...
For $\delta$ =6, the best overlap was found when comparing MDCC and Transbig to Pusztai list. The estimated k is 13, with 12 genes in top-$k$ lists of at least two datasets. In the visualization of Transbig with respect to MDCC, 10 genes are grey-shaded. Most consistent are the ESR1 and TBC1D9 genes, occupying the first two positions in all three lists (distance 0 in both MDCC and TransBig from Pusztai).

\item{\texttt{Summary table} Displays table of objects present in at least one of the top-$k$ lists, as shown in the aggregation map. The table contains object name, position of the object in each of the lists, sum of ranks, frequency in he input lists and frequency in the truncated lists. Highlighted are genes that form the final aggregate list as resulted from the \texttt{TopKSpace}}.
...
The table is ordered in ascending order of the sum of ranks of the objects in the three lists. The first object is gene ESR1, followed by TBC1D9. Genes EVL, SCUBE2, CIRBP and FBP1 are also consistenly present in the top-$k$ truncated lists, but at different positions. Genes FUT8 and RHOB are found ...

\item{\texttt{Venn-diagram and Venn-table}: They show overlaps of objects in the top-$k$ lists of all input lists in the analysis. Highlighted are genes that form the final aggregate list as resulted from the \texttt{TopKSpace}.}
...
all with platform dependence assumption:
<<>>=
bb1=Borda(input,space)
@

Call with top-$k$ assumption:
<<>>=
bb2=Borda(input,input)
@

\subsection{Plotting Borda's scores}
<<fig=T>>=
...
The difference in the Borda scores can be clearly seen in the resulting graph.

\subsection{Run three MC algorithms}
Call with platform dependence assumption
<<>>=
...
Call with the top-$k$ space assumption
<<>>=
...
Generation of the plot of the equilibrium probabilities
<<fig=T>>=
...
\subsection{Run the CEMC algorithm}
Call with platform dependence assumption
<<>>=
set.seed(1234)
...
Call with top-$k$ space assumption
<<>>=
