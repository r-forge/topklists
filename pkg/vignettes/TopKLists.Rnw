...
ranking starts to diminish. For example, Figure~\ref{borda}(a) shows that for the
...
top-$k$ space assumption (Figure~\ref{borda}(b)). \\


<<label=bordaPlot1, include=FALSE>>=
plotBorda(outBorda, K=40, sub="(a)") # plots scores from platform-dependent analysis
@


<<label=bordaPlot2, include=FALSE>>=
plotBorda(outBorda2, sub="(b)") # plots scores from top-k space analysis
@
...
For example, Figure~\ref{equil} shows that 
...
<<deltaplot_complete, echo=TRUE, include=TRUE, pdf=TRUE, fig.pos="H", fig.keep='first', fig.align="center", tidy=TRUE, out.width='9cm', out.height='9cm', fig.cap="Deltaplot for lists MDCC vs. Pusztai">>=
a=deltaplot(breast, deltas = seq(0,300, by=5)) 
@ 
...
<<mydelta2, echo=TRUE, out.width='9cm', out.height='9cm'>>=
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.35]{\Sexpr{gsub("\\\\", replacement="/", tempdir())}/subplotL2-3.pdf}\\
