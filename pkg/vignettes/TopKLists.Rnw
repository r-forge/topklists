...
Whenever you use the package \texttt{TopKLists}, please cite \cite{haschi} and \cite{linding}.\\
...
These three algorithms are implemented in three top-level functions within \texttt{TopKSpace}: \texttt{borda.scores}, \texttt{run,mc}, 
and \texttt{run.cemc}. There are also two plotting functions: \texttt{borda.scores.plot} and \texttt{mc.plot} that help the user to visualize 
...
Four Borda scores are implemented in the \texttt{borda.scores} function of \texttt{TopKSpace}, namely the arithmetic mean (ARM), median (MED),
...
Of the three arguments in \texttt{borda.scores}, only the first one ({\em input}) is required. If argument {\em space} is not
...
<<>>=
outBorda=borda.scores(input,space,k=50) 
# "space" is explicitly specified; underlying space-dependent
@

<<>>=
outBorda1=borda.scores(input) 
#"space" is not specified; all lists are assumed to come from the common space (objects 1-40)

@

<<>>=
outBorda2=borda.scores(input,space=input) 
# "space = input" indicates that this is the top-\emph{k} space
@
...
borda.scores.plot(outBorda, k=40) # plot scores from underlying space-dependent analysis
borda.scores.plot(outBorda2) # plot scores from top-k space analysis
@




Note that the first argument of the \texttt{borda.scores.plot} function is the output from the Borda function, which is 
...
Three Markov Chain (MC) algorithms are implemented in the \texttt{run.mc} function in
...
Of the arguments in \texttt{run.mc}, only the first one, {\em input}, is required. If argument {\em space} is not provided, it is assumed that the underlying space is common among all top-\emph{k} lists, and the union of
...
The Order Explicit Algorithm (OEA) in \cite{linding} is implemented as the \texttt{run.cemc} function in \texttt{TopKSpace}.
Of the three main input arguments in \texttt{run.cemc}, only the first one ({\em input}) is required. If argument {\em space}
...
The default specified in the \texttt{run.cemc} function constitutes a set of 
...
parameters. In fact, the output of \texttt{run.cemc} contains the set of tuning parameters used in the current run. If  users
are unsure of what to use, they are recommended to run \texttt{run.cemc} without setting any parameter and use 
...
<<>>=
set.seed(1234)
outCEMC2 = run.cemc(input, space=input) 
# "space=input" indicates that this is the top-k space.
@
...
Comment 2: From the submatrix shown above for \texttt{run.cemc}, it can be seen that item one is ranked no. 1, since the probability vector in the first column for item one is almost 1, but very small for the rest of the items. Similarly, item 2 is ranked no. 2, and so on.
On the other hand, for \texttt{outCEMC1} with top-\emph{k} space assumption, the 5th item is no longer ranked no. 5, different from the underlying space-dependence results.
...
Comment 3: This file contains all the tuning parameter values (defaults) for running \texttt{run.cemc}. User may
modify this file and use it as the input to the {\em input.par} argument in
the \texttt{run.cemc} function.
...
kendall.plot(input,all.aggregates,space,algorithm=c("ARM","MED","GEO","L2N","MC1","MC2","MC3"))
@
...
\subsection{CEMC on the breast cancer data}
Here the Cross Entropy Monte Carlo method on the breast cancer data is applied (introduced in Section~\ref{sec:introduction}):
<<>>=
data(breast)
@
...
Since each of the top-14 gene lists comes from the common set of 917 genes, one way to set the common underlying space, without loss of information, is to make it the set of all the genes present in these 3 truncated lists. 
...
pd = run.cemc(topK, space, res$maxK)
@
...
Finally, the \texttt{TopKSpace} algorithm for the generation of the aggregate list can be chosen, the default being \texttt{run.cemc}. Last but not least, this panel contains the Calculate button, which starts the analysis.
...
\item{\texttt{Venn-diagram and Venn-table}}:\\ The outputted Venn diagram and Venn table show overlaps of objects in the top-\emph{k} lists of all input lists in the analysis. Asterisk-highlighted genes are objects that form the final aggregate list as returned by the \texttt{TopKSpace} module.
...
\bibitem{desmedt} Desmedt C., Piette F., Loi S., Wang Y. et al. (2007). Strong time dependence of the 76-gene prognostic signature for node-negative breast cancer patients in the TRANSBIG multicenter independent validation series. \emph{Clin Cancer Res} \textbf{1;13(11)}, 3207-2114. PMID: 17545524.
...
\bibitem{shi} Shi L., Campbell G., Jones W.D., Campagne F. et al. (2010). The MicroArray Quality Control (MAQC)-II study of common practices for the development and validation of microarray-based predictive models. \emph{Nat Biotechnol}; \textbf{28(8)}, 827-838. PMID: 20676074.

\bibitem{tabchy} Tabchy A., Valero V., Vidaurre T., Lluch A. et al. (2010). Evaluation of a 30-gene paclitaxel, fluorouracil, doxorubicin, and cyclophosphamide chemotherapy response predictor in a multicenter randomized trial in breast cancer. \emph{Clin Cancer Res} \textbf{1;16(21)}, 5351-5361. PMID: 20829329
