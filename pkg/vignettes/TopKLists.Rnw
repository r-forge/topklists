...
\begin{itemize}
    \item MicroArray Quality Control Phase II Project (2010) - data called \textit{MDACC}
    \item Strong Time Dependence of the 76-Gene Prognostic Signature (2007) - data called \textit{TransBig}
    \item A Clinically Relevant Gene Signature in Triple-Negative and Basal-Like Breast Cancer (2011) - data called \textit{Pusztai}
\end{itemize}
Only genes (unique gene symbols) common to all studies are analyzed, therefore each of the three lists has the length of $N=917$.
...
The aggregation map can only be accessible via \texttt{TopKListsGUI} (see Figure~\ref{aggmap}) and can be characterized as follows: We define an index $p=1,2,\ldots$ and combine $\ell-1$ aggregation levels (groupings of truncated lists) in one display: 
...
The Venn diagram can only be accessible via \texttt{TopKListsGUI} (see Figure~\ref{venn}).
...
data(breast)
res = j0.multi(breast, d=6, v=10)
t = as.list(breast[1:res$maxK, ])
pd = CEMC(t, res$maxK, t)
pd$topK
@
